'use client'

import Image from "next/image";
import { CartItemWithProduct } from "../lib/cart";
import Link from "next/link";
import formatPrice from "../lib/format";
import { setProductQuantity } from "./action";
import { useTransition } from "react";

interface CardEntryProps {
    cartItem: CartItemWithProduct,
    setProductQuantity: (productId:string, quantity:number) => Promise<void>
}

const CartEntry = ({cartItem: {id, product, quantity}, setProductQuantity}: CardEntryProps) => {
    const quantityOptions: JSX.Element[] = []
    for (let i = 1; i < 100; i++) {
        quantityOptions.push(
            <option value={i} key={i}>{i}</option>
        );     
    }
    
    const [isPending, startTransition] = useTransition();
    // ini supaya server action bisa dilakukan di client side. Ditambah fitur isPending nya membuat kita bisa membuat fitur loading-spinner

    return ( 
        <div>
            <div className="flex flex-wrap items-center gap-3">
                <Image src={product.imageURL} alt={product.name} width={200} height={200} className="rounded-lg"/>
                <div>
                    <Link href={`/product/${product.id}`} className="font-bold">{product.name}</Link>
                    <div>Price: {formatPrice(product.price)}</div>
                    <div className="my-1 flex items-center gap-2">
                        Quantity:
                        <select name="quantity" className="select select-bordered w-full max-w-[80px]" defaultValue={quantity}
                            onChange={(e)=>{
                                const newQuantity = parseInt(e.currentTarget.value)
                                startTransition( async () => {
                                    await setProductQuantity(id, newQuantity)
                                })
                            }}
                        >
                            {quantityOptions}
                        </select>
                    </div>
                    <div className="flex items-center gap-2">
                        Total: {formatPrice(product.price * quantity)}
                    </div>
                    {isPending && <span className="loading loading-spinner loading-md"></span>}
                </div>
            </div>
            <div className="divider"></div>
        </div>
     );
}
 
export default CartEntry;